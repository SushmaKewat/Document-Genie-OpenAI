...
def get_document_splits_with_ids(text_chunks):
    document_ids = []
    
    for page, chunks in groupby(text_chunks, lambda chunk : chunk.metadata['page']):
...
def get_conversational_chain(api_key):
    ...
    Answer the question/query as detailed as possible from the provided context, make sure to provide all the details. 
    If the user asks for the summary of the document, go through the contents of the document and give your response. 
    If you don't find the answer, go through the context again, even then
    if the answer is not in provided context just say, "answer is not available in the context", don't provide the wrong answer and do not leave any answer unfinished. Always provide full answer and complete the sentence.
    \n\n
    Context:\n {context}\n
    Question: \n{question}\n
    ...
    print("STREMLIT CHAT HISTORY: ", history)
    memory = ConversationBufferMemory(chat_memory = history, input_key='question') # store the history in the memory
    print("MEMORY: ", memory)
    ...
    model = ChatOpenAI(temperature=0.3, openai_api_key=api_key)
    
    prompt = PromptTemplate(template=prompt_template, input_variables=["context", "question"])
    
    chain = load_qa_chain(llm=model, chain_type="stuff", memory=memory, prompt=prompt)
